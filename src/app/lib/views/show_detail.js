    ...
    var ShowDetail = Marionette.View.extend({
        ...
        initialize: function () {
            ...
            _.each(this.model.get('episodes'), function (value, currentEpisode) {
                ...
                torrents[value.season][1000000 - parseInt(value.episode)] = value;
            });
        ...
        onAttach: function () {
            ...
            var poster = this.model.get('poster') || images.poster || noimg;
            if (!backdrop) {
            ...
            Database.getEpisodesWatched(this.model.get('tvdb_id'))
                .then(function (data) {
                    _.each(data, function (value, state) {
                        ...
                        if (value) {
                            episodesSeen.push(parseInt(value.season) * 5000 + parseInt(value.episode));
                        }
                    });
                    var season = 1;
                    var episode = 1;
                    if (episodesSeen.length > 0) {
                        ...
                        _.each(_this.model.get('episodes'),
                            function (value, currentepisode) {
                                episodes.push(parseInt(value.season) * 5000 + parseInt(value.episode));
                            }
                        );
                        ...
                        if (AdvSettings.get('tv_detail_jump_to') !== 'firstUnwatched') {
                            ...
                            if (lastSeen !== episodes[episodes.length - 1]) {
                                ...
                                if (!idx) {
                                    ...
                                    unseen.push(first);
                                    episode = unseen[0] % 5000;
                                    season = (unseen[0] - episode) / 5000;
                                } else {
                                    var next_episode = episodes[idx + 1];
                                    episode = next_episode % 5000;
                                    season = (next_episode - episode) / 5000;
                                }
                            } else {
                                episode = lastSeen % 5000;
                                season = (lastSeen - episode) / 5000;
                            }
                        } else {
        ...
        selectEpisode: function ($elem) {
            ...
            var torrents = {};
            var quality;
            torrents.q360 = $('.template-' + tvdbid + ' .q360').text();
            ...
            this.ui.q360p.removeClass('active');


            if (!torrents.q360) {
                this.ui.q360p.addClass('disabled');
            } else {
                this.ui.q360p.removeClass('disabled');
            }
            if (!torrents.q480) {
            ...
            if (quality === '1080p') {
                ...
                torrents.def = torrents.q360;
                torrents.quality = '360p';
                this.ui.q360p.addClass('active');
            }
